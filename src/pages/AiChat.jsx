        ...
        <div className="min-h-screen bg-slate-50 font-bengali flex flex-col md:flex-row relative">
            ...
            <div className="flex-1 flex flex-col h-screen md:pl-0 relative z-0">
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 bg-slate-50/50 scroll-smooth">
                        ...
                        >
                            <div className={`flex items-end gap-3 max-w-[85%] md:max-w-[70%] ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.sender === 'user' ? 'bg-indigo-100' : 'bg-emerald-100'}`}>
                                    {msg.sender === 'user' ? <User size={16} className="text-indigo-600" /> : <img src="/assets/ai.png" alt="AI" className="w-full h-full rounded-full object-cover" />}
                                </div>
                                <div className={`p-4 md:p-5 rounded-2xl shadow-sm text-sm md:text-base leading-relaxed ${msg.sender === 'user'
                    ...
                    {/* Default Questions - Only show if just 1 message (greeting) exists or at bottom to prompt user */}
                    {messages.length < 2 && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4 max-w-3xl mx-auto">
                            {quickQuestions.map((q, idx) => (
                                <button
                                    key={idx}
                                    onClick={() => handleSendMessage(q)}
                                    className="text-left p-4 bg-white border border-slate-200 hover:border-emerald-500 text-slate-600 hover:text-emerald-700 rounded-xl transition-all shadow-sm hover:shadow-md text-sm font-bold"
                                >
                                    {q}
                                </button>
                            ))}
                ...
                <div className="p-4 bg-white border-t border-slate-200 shrink-0">
                    <div className="max-w-4xl mx-auto relative flex items-center gap-3">
                        <input
                            ...
                            className="flex-1 bg-slate-50 border-2 border-slate-200 rounded-full px-6 py-4 outline-none focus:border-emerald-500 focus:bg-white transition-all font-bold text-slate-700 placeholder:text-slate-400"
                        />
                        <button
                            onClick={() => handleSendMessage(inputText)}
                            className="bg-emerald-600 hover:bg-emerald-700 text-white p-4 rounded-full shadow-lg transition-transform active:scale-90"
                        >
                            <Send size={20} />
                        </button>
                    </div>
