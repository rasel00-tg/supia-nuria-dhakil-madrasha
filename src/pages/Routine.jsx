...
const Routine = () => {
    ...
    // Filter Logic - Safe check for routine existence
    ...
    if (loading) {
        return (
            <div className="fixed inset-0 z-50 bg-white/80 backdrop-blur-md flex items-center justify-center font-bengali">
                <div className="flex items-center gap-3 animate-pulse">
                    <img src={logo} alt="Loading" className="w-12 h-12 object-contain" />
                    <h1 className="text-xl font-bold text-slate-800">সুফিয়া নূরীয়া দাখিল মাদ্রাসা</h1>
                </div>
            </div>
        )
    }
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-bengali pb-40">
                ...
                <div className="max-w-7xl mx-auto flex justify-between items-center gap-3">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className={`flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-lg font-bold transition-all shadow-md active:scale-95 whitespace-nowrap text-sm md:text-base ${!selectedClass ? 'opacity-50 cursor-not-allowed' : 'hover:bg-emerald-500'}`}
                        >
                            <Printer size={16} />
                        ...
                        <button
                            ...
                            disabled={!selectedClass}
                            className={`flex items-center gap-2 bg-slate-800 text-white px-4 py-2 rounded-lg font-bold transition-all shadow-md active:scale-95 whitespace-nowrap text-sm md:text-base ${!selectedClass ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-700'}`}
                        >
                            <Download size={16} />
            ...
            {/* Landing Content & Selection Area */}
            <div className="max-w-4xl mx-auto px-4 mb-8 flex flex-col items-center text-center no-print">
                {/* Logo & Name Inline */}
                <div className="flex items-center justify-center gap-3 mb-4">
                    <img src={logo} alt="Madrasha Logo" className="w-12 h-12 md:w-16 md:h-16 object-contain" />
                    <h1 className="text-xl md:text-3xl font-black text-slate-900">সুফিয়া নূরীয়া দাখিল মাদ্রাসা</h1>
                </div>

                {/* Title */}
                <h2 className="text-xl md:text-2xl font-bold text-emerald-700 mb-2">অনলাইন রুটিন সিস্টেম</h2>

                {/* Instruction */}
                <p className="text-slate-600 font-bold mb-6">অনুগ্রহ করে আপনার রুটিন খুঁজতে ক্লাস সিলেক্ট করুন</p>

                {/* Class Selector Trigger */}
                <button
                    onClick={() => setShowClassPopup(true)}
                    className="flex items-center justify-between gap-4 bg-white px-6 py-3 rounded-xl border-2 border-slate-200 shadow-sm font-bold text-slate-700 hover:border-emerald-500 hover:text-emerald-700 transition-all min-w-[280px]"
                >
                    <span>{selectedClass || 'ক্লাস সিলেক্ট করুন'}</span>
                    <ChevronDown size={20} className="text-slate-400" />
                </button>
            </div>

            {/* Main Content Area (Hidden until class selected) */}
            {selectedClass && (
                <div className={`w-full flex justify-center py-2 md:py-4 print-wrapper ${scale === 1 ? 'overflow-auto' : 'overflow-hidden'}`}>
                    ...
                    >
                        <div className="flex-1">
                            ...
                            {/* 4. Special Instructions (Renamed title) */}
